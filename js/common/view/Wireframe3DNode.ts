...
export default class Wireframe3DNode extends Node {
  ...
  public constructor( wireframeModel: Wireframe3D, providedOptions: WireframeNodeOptions ) {
    ...
    this.path = new Path( null, {
      stroke: options.stroke, //TODO use color palette (or alpha?) based on z coordinate
  ...
  public update(): void {
    ...
    this.wireframeModel.lines.forEach( line => {
      const vertex1 = this.wireframeModel.getTransformedVertex( line.vertex1Index );
      const vertex2 = this.wireframeModel.getTransformedVertex( line.vertex2Index );
